...
class CameraCalibrationDataCharuco:
    def __init__(self, uid):
        self.uid = uid
        self.image_size = None
        self.frames = []
    ...
    def to_string(self, indentation=0):
        ...
        inner_indent = "\t"*(indentation+1)
        return "\n".join([
            f"{base_indent}CameraCalibrationDataCharuco({self.uid}))",
            f"{inner_indent}image_size:  {self.image_size}",
            f"{inner_indent}frame_count: {len(self.frames)}",
            f"{inner_indent}corner_count:{len(self.all_corners)}",
            f"{inner_indent}id_count:    {len(self.all_ids)}",
            f"{inner_indent}ip_count:    {len(self.image_points)}",
            f"{inner_indent}op_count:    {len(self.object_points)}",
        ])
...
class StereoCalibrationDataCharuco:
    def __init__(self, uid=None, left_uid=None, right_uid=None):
        self.uid = uid
        self.image_size = None
        self.left = CameraCalibrationDataCharuco(left_uid)
        self.right = CameraCalibrationDataCharuco(right_uid)
        self.object_points = []
    def to_string(self, indentation=0):
        base_indent = "\t"*indentation
        inner_indent = "\t"*(indentation+1)
        return "\n".join([
            f"{base_indent}StereoCalibrationCharuco({self.uid}))",
            f"{inner_indent}image_size:  {self.image_size}",
            f"{inner_indent}op_count:    {len(self.object_points)}",
            f"{inner_indent}left:",
            self.left.to_string(indentation+2),
...
def capture_calibration_data_charuco(load_images=False, save_images=False):
    calibration_data = StereoCalibrationDataCharuco(uuid.uuid4().hex, uuid.uuid4().hex, uuid.uuid4().hex)
    l_viz = None
    r_viz = None
    capturing = True
    if load_images:
    ...
    for img in iterable:
        ...
        l_corners, l_ids, l_rejectedImgPoints = cv2.aruco.detectMarkers(l_gray, settings.ARUCO_DICT)
        r_corners, r_ids, r_rejectedImgPoints = cv2.aruco.detectMarkers(r_gray, settings.ARUCO_DICT)

        # ic()
        if len(l_corners) > 0 and len(r_corners) > 0:
            l_better_corners = []
            for corner in l_corners:
                l_better_corners.append(cv2.cornerSubPix(l_gray, corner,
                                                        winSize = (3,3),
                                                        zeroZone = (-1,-1),
                                                        criteria = settings.CHARUCO_SUBPIXEL_CRITERIA))
            l_marker_count, l_refined_corners, l_refined_ids = cv2.aruco.interpolateCornersCharuco(l_better_corners,l_ids,l_gray,settings.CHARUCO_BOARD)
            r_better_corners = []
            for corner in r_corners:
                r_better_corners.append(cv2.cornerSubPix(r_gray, corner,
                                                        winSize = (3,3),
                                                        zeroZone = (-1,-1),
                                                        criteria = settings.CHARUCO_SUBPIXEL_CRITERIA))
            r_marker_count, r_refined_corners, r_refined_ids = cv2.aruco.interpolateCornersCharuco(l_better_corners,l_ids,l_gray,settings.CHARUCO_BOARD)

            if l_refined_corners is not None \
            and l_refined_ids is not None \
            and len(l_refined_corners)>20 \
            and l_marker_count > 20 \
            and r_refined_corners is not None \
            and r_refined_ids is not None \
            and len(r_refined_corners)>20 \
            and r_marker_count > 20:
                    
                cv2.imshow('calib',np.hstack([
                    cv2.aruco.drawDetectedMarkers(
                        image=np.ascontiguousarray(l_img), 
                        corners=l_corners),
                    cv2.aruco.drawDetectedMarkers(
                        image=np.ascontiguousarray(r_img), 
                        corners=r_corners)
                ]))
                calibration_data.left.all_corners.append(l_refined_corners)
                calibration_data.left.all_ids.append(l_refined_ids)
                calibration_data.right.all_corners.append(r_refined_corners)
                calibration_data.right.all_ids.append(r_refined_ids)
                
                l_image_points = None
                r_image_points = None
                l_object_points = None
                r_object_points = None
                for i in range(l_refined_ids.shape[0]):
                    index = l_refined_ids[i][0]
                    if l_image_points is None:
                        l_image_points = l_refined_corners[i][0]
                        l_object_points = settings.CHARUCO_BOARD.chessboardCorners[index]
                    else:
                        l_image_points = np.vstack([l_image_points, l_refined_corners[i][0]])
                        l_object_points = np.vstack([l_object_points, settings.CHARUCO_BOARD.chessboardCorners[index]])
                for i in range(r_refined_ids.shape[0]):
                    index = r_refined_ids[i][0]
                    if r_image_points is None:
                        r_image_points = r_refined_corners[i][0]
                        r_object_points = settings.CHARUCO_BOARD.chessboardCorners[index]
                    else:
                        r_image_points = np.vstack([r_image_points, r_refined_corners[i][0]])
                        r_object_points = np.vstack([r_object_points, settings.CHARUCO_BOARD.chessboardCorners[index]])
                
                calibration_data.left.image_points.append(l_image_points)
                calibration_data.right.image_points.append(r_image_points)
                calibration_data.left.object_points.append(l_object_points)
                calibration_data.right.object_points.append(r_object_points)
                calibration_data.object_points.append(l_object_points)
            ...
            key = cv2.waitKey(33)
        if len(calibration_data.left.all_corners)> 1000 or key != -1:
            if key == 27:
                ic("exitting")
                cv2.destroyAllWindows()
                return None
...
def run_stereo_calibration_charuco(calibration_data:StereoCalibrationDataCharuco):
    ...
    calibration.left.distortion_coeffs, \
    calibration.left.rotations, \
    calibration.left.translations = cv2.aruco.calibrateCameraCharuco(
        charucoCorners=calibration_data.left.all_corners,
        charucoIds=calibration_data.left.all_ids,
        board=settings.CHARUCO_BOARD,
        imageSize=calibration_data.image_size,
        cameraMatrix=None,
        distCoeffs=None)
    ...
    calibration.right.distortion_coeffs, \
    calibration.right.rotations, \
    calibration.right.translations = cv2.aruco.calibrateCameraCharuco(
        charucoCorners=calibration_data.right.all_corners,
        charucoIds=calibration_data.right.all_ids,
        board=settings.CHARUCO_BOARD,
        imageSize=calibration_data.image_size,
        cameraMatrix=None,
        distCoeffs=None)
...
def capture_data_charuco():
    calibration_data = capture_calibration_data_charuco()
    if calibration_data is None:
        return
    ic()
    with open(settings.EXPORT_DATA_FILENAME_CHARUCO, "wb") as fp:
        pickle.dump(calibration_data, fp)
...
def load_data_and_calibrate_charuco():
    with open(settings.IMPORT_DATA_FILENAME_CHARUCO, "rb") as fp:
        calibration_data = pickle.load(fp)

    print("calibration data ready")
    print(calibration_data.to_string())

    calibration = run_stereo_calibration_charuco(calibration_data)
    with open(settings.EXPORT_CALIBRATION_FILENAME_CHARUCO, "wb") as fp:
        pickle.dump(calibration, fp)
...
def capture_data_and_calibrate_charuco():
    calibration_data = capture_calibration_data_charuco()
    if calibration_data is None:
        return
    with open(settings.EXPORT_DATA_FILENAME_CHARUCO, "wb") as fp:
...
def print_stored_data_and_calibration_charuco():
    with open(settings.IMPORT_DATA_FILENAME_CHARUCO, "rb") as fp:
        calibration_data = pickle.load(fp)
    ...
    with open(settings.IMPORT_CALIBRATION_FILENAME_CHARUCO, "rb") as fp:
        calibration = pickle.load(fp)
